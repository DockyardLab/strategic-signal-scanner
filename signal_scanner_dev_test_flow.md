...
这说明真正的生产问题不只在代码，也可能在域名、DNS 和邮件服务。

---

## 9. 第八阶段：用 Cloud Shell 做最后验证

### 9.1 手动执行 Cloud Run Job

在 Cloud Shell 里我们用：

```bash
gcloud run jobs execute strategic-signal-scanner --region asia-east1 --wait
```
...
先把逻辑拆成可测单元，再搬到 Cloud Run。

### 10.2 先跑通，再变美

先确认链路闭环，再优化 UI、archive 和排版。
...
这次 `Strategic Signal Scanner` 的完整流程是：

> 先做本地原型，再迁移 Cloud Run；先跑通抓取、评分、report、archive，再补 state 持久化、去重、归档和邮件；最后用 Cloud Shell 做上线前验证，确认定时触发、自动发信和历史归档都正常。
